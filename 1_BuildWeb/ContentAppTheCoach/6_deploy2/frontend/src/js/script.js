...
async function generateLearningFlexible(lessons) {
    // Transform lessons data to match the flexible prompt format
    const flexibleLessons = lessons.map(lesson => ({
        question: lesson.question || "Which company are you working for?", // default question if not provided
    ...
    try {
        showLoadingDialog();
        console.log('Sending flexible lessons to API:', flexibleLessons);
